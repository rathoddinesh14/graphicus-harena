...
\begin{tikzpicture}[
    node distance=1.2cm,
    % Renamed 'out' to 'displayout' to avoid conflict with TikZ keyword
    ...
    displayout/.style={rectangle, rounded corners, draw=red!60!black, fill=red!10, thick, text width=3.5cm, align=center, font=\sffamily\small}
]
  ...
  \node[displayout, below=of fs] (canvas) {Canvas Display};
...
\begin{itemize}
    \item \textbf{Rasterization:} The GPU determines which pixels on screen cover the geometry (the "proxies" for our volume).
    \item \textbf{Ray Marching:} For every fragment, the shader samples the 3D texture along the camera's view ray to compute the final pixel intensity.
\end{itemize}
\end{frame}
